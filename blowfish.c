...
uint32_t P[18] = {0x243f6a88, 0x85a308d3, 0x13198a2e, 0x03707344, 0xa4093822, 0x299f31d0, 0x082efa98, 0xec4e6c89, 0x452821e6, 0x38d01377, 0xbe5466cf, 0x34e90c6c, 0xc0ac29b7, 0xc97c50dd, 0x3f84d5b5, 0xb5470917, 0x9216d5d9, 0x8979fb1b};     // P-array
uint32_t S[4][256] = {
...
int main(){
	uint32_t key[] = {1, 1, 2, 10, 4, 100, 8, 1000, 16, 10000, 32, 100000};
	key_schedule(key, 12);
	printf("Encrypt(1) or decrypt(2)? ");
	int behavior;
	scanf("%1d", &behavior);
	if(behavior != 1 && behavior != 2){
		printf("Invalid behavior!");
		return 1;
	}
	char ifilename[200];
	printf("Where's the input? ");
	scanf("%s", ifilename);
	char ofilename[200];
	printf("Where's the output? ");
	scanf("%s", ofilename);
	FILE *ifile = fopen(ifilename, "r");
	FILE *ofile = fopen(ofilename, "w");
	if(ifile == NULL){
		printf("Can't do it! ifile bad\n");
		return 2;
	}
	if(ofile == NULL){
		printf("Can't do it! ofile bad\n");
		return 3;
	}
	char *buff = malloc(64);
	char *buffL = malloc(32);
	...
	while(bread > 0){
		...
		buff[6] = buffR[2];
		buff[7] = buffR[3];
		fwrite(buff, 1, 8, ofile);
		bread = fread(buff, 1, 8, ifile);
	}
	fclose(ifile);
	fclose(ofile);
	return 0;
