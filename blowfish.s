main:
	li $s0, 1				#initialize behavior to 1(encrypt)
	la $a0, inputprompt		#load our prompt for input here
...
swapendian:					#takes a0 as swapthis
	#Comments that better explain what I'm about to do can be found in dencrypt. Sorry.to make
	addi $sp, $sp, -20		#move the stack pointer down for heap stuff
	sw $t0, ($sp)
	sw $t1, 4($sp)
	sw $t2, 8($sp)
	sw $t3, 12($sp)
	sw $a0, 16($sp)
	#done storing
	lb $t0, 19($sp)			#tmp[0] = swapthis[3]
	lb $t1, 18($sp)			#tmp[1] = swapthis[2]
	lb $t2, 17($sp)			#tmp[2] = swapthis[1]
	lb $t3, 16($sp)			#tmp[3] = swapthis[0]
	sb $t0, 16($sp)			#swapthis[0] = tmp[0]
	sb $t1, 17($sp)			#swapthis[1] = tmp[1]
	sb $t2, 18($sp)			#swapthis[2] = tmp[2]
	sb $t3, 19($sp)			#swapthis[3] = tmp[3]
	#begin restoring
	lw $t0, ($sp)
	lw $t1, 4($sp)
	lw $t2, 8($sp)
...
f:							#takes a0 as "x"
	add $s7, $zero, $ra		#copy ra into s7 so we can jump safely to swapendian
	jal swapendian			#swap byte order because we're in little endian on intel
	add $ra, $zero, $s7		#copy it back
...
dencrypt:					#takes a0 as both the source and destination(we put it where we got it), and takes a1 as the length in bytes of the data(after padding)
	#For sake of ease, we're going to store the values of all t and s registers, other than s4, and ra in the heap.
	...
	addi $sp, $sp, -92				#92 bytes for all registers we plan to store
	sw $t0, ($sp)
	sw $t1, 4($sp)
	sw $t2, 8($sp)
	sw $t3, 12($sp)
...
denloop:
		...
		sw $t0, 72($sp)			#prevent data corruption
		sw $t1, 76($sp)			#prevent data corruption
		sw $t2, 80($sp)			#prevent data corruption
		sw $t3, 84($sp)			#prevent data corruption
		sw $a0, 88($sp)			#prevent moar data corruption
...
denlr:
		lw $t0, 72($sp)			#bring back values
		lw $t1, 76($sp)			#bring back values
		lw $t2, 80($sp)			#bring back values
		lw $t3, 84($sp)			#bring back values
		lw $a0, 88($sp)			#bring back moar values
...
encryptblock:				#takes a2 as "L" and a3 as "R".
...
decryptblock:				#takes a2 as "L" and a3 as "R".
	addu $s2, $zero, $ra	#copy ra into s2 so we can jump to other functions while here and still get back correctly
...
ksl2:	beq $t0, $t1, endkl2	#jump to the end of the loop if we've finished
		...
		addu $s4, $zero, $a0	#copy a0 to s4 for same reason
		jal encryptblock		#encrypt
		addu $t0, $zero, $s3	#copy it back
		addu $a0, $zero, $s4	#copy it back
		li $t1, 18				#for when we need to loop check again
